/*
 * File:   Game.c
 * Author: Jan Ritter & Fabian Patzwall
 *
...
bool placeShipShoot(int x, int y, board* field) {
    ...
    bool isNotAllowed = false;
    for (int i = -1; i <= 1 && !isNotAllowed; i++) {
        for (int j = -1; j <= 1 && !isNotAllowed; j++) {
            int posX = x + i;
            int posY = y + j;
            // Überprüfen, ob das Nachbarfeld innerhalb des Spielfelds liegt und ob es besetzt ist
            if (posX >= 0 && posX < ROWS && posY >= 0 && posY < COLS && (*field)[posX][posY] == SUNK) {
                // Mindestens ein Nachbarfeld ist besetzt, daher kann das Schiff nicht platziert werden
                isNotAllowed = true;
            }
        }
    }
    if ((*field)[y][x] == EMPTY && !isNotAllowed){
        return true;
    }
    return false;
}
...
void updatePosition(int shipLength, board* field) {
   ...
   if (isRotated != isRotatedPrev || x != xPrev || y != yPrev || (x + y == 0)){
        ...
        // Vertical placement
        for (int j = 0; j < shipLength && (x + j < COLS); j++) {
...
bool isSunk(board* field, int x, int y){
    ...
    int offset = 1;
    int positions[8] = {0}; //max Schifflänge * 2 Koordinaten = 8
